...
函式也可以被定義成擁有*參數（parameters）*的，這是函式（signatures）中特殊的變數。當函式有參數時，你可以提供那些參數的確切數值。嚴格上來說，我們傳遞的數值會叫做*引數（arguments）*。但爲了方便起見，通常大家不太會去在意兩者的區別。雖然函式定義時才叫*參數*，傳遞數值時叫做*引數*，但很多時候會被人們拿來交互使用。
