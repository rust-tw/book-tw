...
Rust 的記憶體安全保障讓意外產生永遠不會清除的記憶體的情況（通稱為**記憶體泄漏／memory leak**）難以發生，但這並不是絕對不可能。和在編譯期間不允許資料競爭的情況不同，Rust 並不保證能完全避免記憶體泄漏，代表記憶體泄漏 Rust 中被視為是記憶體安全的。我們可以透過使用 `Rc<T>` 和 `RefCell<T>` 觀察到 Rust 仍會產生記憶體泄漏，產生引用循環是有可能的，這會導致記憶體泄漏，因為循環中的引用計數永遠不會變成 0，所以數值永遠不會被釋放。
