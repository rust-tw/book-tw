...
Rust 的記憶體安全保障讓意外產生永遠不會清除的記憶體的情況（通稱為**記憶體泄漏／memory leak**）難以發生，但這並不是絕對不可能。Rust 不保障完全避免記憶體泄漏，對 Rust 來說記憶體泄漏是記憶體安全的。我們可以透過使用 `Rc<T>` 和 `RefCell<T>` 觀察到 Rust 仍會產生記憶體泄漏，產生引用循環是有可能的，這會導致記憶體泄漏，因為循環中的引用計數永遠不會變成 0，所以數值永遠不會被釋放。
