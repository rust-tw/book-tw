...
此程式碼有點雜亂，但已經足夠作爲模擬用途了。我們建立了另一個請求 `sleep`，且我們的伺服器能識別其資料。我們在 `if` 區塊後加上 `else if` 來檢查 `/sleep` 的請求。當收到請求時，伺服器將會在成功回傳 HTML 頁面前沉睡 5 秒鐘。
...
現在錯誤的原因是因爲我們的 `ThreadPool` 沒有 `execute` 方法。回想一下[「用有限數量的執行緒建立類似介面」](#用有限數量的執行緒建立類似介面)<!-- ignore -->段落中，我們決定我們的執行緒池要有類似於 `thread::spawn` 的介面。除此之外，我們會實作 `execute` 函式使其接收給予的閉包並傳至執行緒池中閒置的執行緒來執行。
