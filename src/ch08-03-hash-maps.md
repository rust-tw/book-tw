...
的資料型別推導出雜湊映射的型別。在範例 8-21 中的鍵型別就會是 `String` 然後值的型別就會是 `i32`，如同範例 8-20 一樣。
...
此程式碼會印出 `{"world": 2, "hello": 1, "wonderful": 1}`。`split_whitespace` 方法會遍歷 `text` 中被空格分開來的切片。`or_insert` 方法會回傳該鍵對應數值的可變引用（`&mut V`）。在此我們將可變引用儲存在 `count` 變數中，所以要賦值的話，我們必須先使用 `*` 來解引用（dereference）`count`。可變引用會在 `for` 結束時離開作用域，所以所有的改變都是安全的且符合借用規則。
