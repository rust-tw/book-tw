...
不同程式語言會以不同的方式實作執行緒。許多作業系統都有提供 API 來建立新的執行緒。這樣讓一個語言呼叫做業系統 APIs 來建立執行緒的方式有時會稱為 *1:1*，代表每一個語言產生的執行緒就是一個作業系統的執行緒。
...
在這裡我們指的**執行時**是每個執行檔中語言所提供的程式碼。此程式碼根據不同語言可大可小，但每個非組合語言的語言都一定會有一些執行時程式碼。所以說當人們常說一個語言「沒有執行時」的話，他們常常指的是「小執行時」。較小的執行時提供的功能就較少，但優勢在於可以產生較小的執行檔，這能使得與其他語言結合更加容易。雖然許多語言都能接受增加執行時的大小來換取更多功能，但 Rust 需要做到幾乎沒有執行時，且必須能夠呼叫 C 來維持效能，這也是不能妥協的。
...
如果我們在範例 16-45 對主執行緒的 `drop` 呼叫前使用 `move` 閉包會發生什麼事呢？`move` 能修正這樣的情況嗎？很不幸的，不行。我們會得到不同的錯誤，因為範例 16-4 想做的事情會因為不同原因而不被允許。如果我們對閉包加上了 `move`，我們將會把 `v` 移入閉包環境，而在主執行緒將無法再對它呼叫 `drop` 了。我們會得到另一個編譯錯誤：
