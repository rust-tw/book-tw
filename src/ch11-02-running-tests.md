...
就像 `cargo run` 會編譯你的程式碼並執行產生的執行檔，`cargo test` 會在測試模式編譯你的程式碼並執行產生的測試執行檔。你可以指定命令列選項來改變 `cargo test` 的預設行為。舉例來說 `cargo test` 預設行為產生的執行檔會平行執行所有測試並獲取測試執行時產生的輸出，讓測試各自的輸出結果不會顯示出來，以更容易讀取相關測試的結果。
