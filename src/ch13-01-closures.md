...
我們的下一個範例──儲存閉包並在之後才執行──是很實用的情況。在過程中，我們會討論到閉包語法、型別推導以及特徵。
...
低強度重訓方案會依據我們模擬的複雜演算法來建議伏地挺身和仰臥起坐的次數。

如果使用者想要高強度的重訓，就會有額外的邏輯：如果應用程式產生的隨機數字是 3 的話，應用程式會建議休息並多喝水；如果不是的話，使用者會依據複雜演算法得到數分鐘的跑步訓練。
...
我們想在 `generate_workout` 內只用一次 `simulated_expensive_calculation`，且只在我們真的需要結果的地方進行昂貴的運算就好。這就是閉包能使用的場合！
...
現在如何進行耗時的計算都定義在同一處了，而且我們只會在需要結果時才會執行該程式碼。
