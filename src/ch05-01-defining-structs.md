...
你也可以定義沒有任何欄位的結構體！這些叫做*類單元結構體（unit-like structs）*，因爲它們的行爲就很像 `()` 單元型別（unit type）。類單元結構體很適合用在當你要實作一個特徵（trait）或某種型別，但你沒有任何需要儲存在型別中的資料。我們會在第十章討論特徵。

> ### 結構體資料的所有權
>
> 在範例 5-1 的 `User` 結構體定義中，我們使用了擁有所有權的 `String` 型別，而不是 `&str` 字串切片型別。這邊是故意這樣選擇的，因爲我們希望解構體的實例可以擁有它所有的資料，並在整個結構體都有效時資料也是有效的。
>
